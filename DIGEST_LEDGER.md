...
| 2 | 2026-08-23 | 1 | — | — | Same Sept 2 bullet, now "unresolved for 1 day". Still correct (unassigned as of 08-23 review). Chat check 08-23: a host asked Itzel on 08-22 08:41 ("free before 3pm Aug 31? and Sept 2?"), no reply yet — the bullet can't say so because the facts schema has no `question` kind (extracted as 0.75-confidence `schedule_assertion`, below the 0.85 gate). **Fixed same day as 1.41.0/1.41.1 (`_outreach` detector + bot): the 17:44 manual digest read "waiting on Itzel — asked Aug 22 (1 day) — nudge in a couple of days".** A fifth cause shape, not in the tag set: *state-never-computed* — the Pi held the fact and no detector read it. ⚠️ Watch nights 3–5: `days_waiting` must climb +1/night for Sept 2 (a reset with no new ask = wrong fact resolving); on the night it reaches 3 the bullet must leave `wait` and become a nudge; any `declined` bullet gets read by hand (rarest branch); a bullet that names a cleaner nobody asked = floor too low → set `outreach_min_confidence: 0.85` in the add-on options, no redeploy. Observation, not a flag: tomorrow's Aug 24 Darya clean has no agreed time (`time_unagreed`, suggest) — announced once on 08-10 and silent since, because `suggest` never repeats and this detector has no urgency promotion as the date nears (unlike `drift_unassigned` at 30d). |
| 3 | 2026-08-24 | | | | |
| 4 | 2026-08-25 | | | | |
| 5 | 2026-08-26 | | | | |
| 6 | 2026-08-27 | | | | |
